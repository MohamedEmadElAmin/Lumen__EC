...
----
# Instructions
##### 1). Adding new json files (DataProvider)
You can add any number of files with any name , any extension (all extensions except .json will ignored) at the following path 
```sh
database/json/
```

If the new files had new keywords 
ex: myId, open
```sh
app/Helper/ProviderMaps.php
```
and add the new keywords to its corresponding words.
...
```sh
api/v1/transactions?statusCode=paid&currency=EGP&provider=DataProviderY&amountMin=1&amountMax=600
```
